...
unsigned int serverPort = 8080; // Pour le serveur
unsigned int clientPort = 8081; // Pour le client


IPAddress serverIP(10, 1, 224, 86); // Adresse IP de l'ESP32 Serveur
//IPAddress allowedClientIP(10, 1, 224, 36); // IP spécifique autorisée (carte de I)
IPAddress allowedClientIP(10, 1, 224, 135); // IP spécifique autorisée (Carte de Art)
  ...
  while (true) {
    ...
    while (keypad.available()) {
      ...
      if (e.bit.EVENT == KEY_JUST_PRESSED) {
        if (e.bit.KEY == 'A') { // Choix du Serveur
          isServer = true;
          ...
          while (true) {
            ...
            if (packetSize) {
              ...
              if (udp.remoteIP() == allowedClientIP) {
          ...
          break; 
        } else if (e.bit.KEY == 'B') { // Choix du Client
          isServer = false;
          ...
          // Client envoie un message d'initialisation au serveur
            snprintf(outgoingPacket, 255, "Hello Server");
            udp.beginPacket(serverIP, serverPort);
            udp.write((uint8_t*)outgoingPacket, strlen(outgoingPacket));
            udp.endPacket();
...
void loop() {
  ...
  while (keypad.available()) {
    keypadEvent e = keypad.read();
    if (e.bit.EVENT == KEY_JUST_PRESSED) {
      if (e.bit.KEY == 'B') { // Bouton gauche pressé
        leftButtonPressed = true;
      } else if (e.bit.KEY == 'A') { // Bouton droit pressé
        rightButtonPressed = true;
      }
    } else if (e.bit.EVENT == KEY_JUST_RELEASED) {
      if (e.bit.KEY == 'B') { // Bouton gauche relâché
        leftButtonPressed = false;
      } else if (e.bit.KEY == 'A') { // Bouton droit relâché
  ...
  if (isServer) {
    ...
    if (ballY >= SCREEN_HEIGHT - 4 && ballX >= paddleX && ballX <= paddleX + 10) { // Correction: augmentation de la zone de collision
        ballSpeedY *= -1;
    ...
    if (ballY <= 4 && ballX >= opponentPaddleX && ballX <= opponentPaddleX + 10) { // Correction: augmentation de la zone de collision
        ballSpeedY *= -1;
        ballY = 5;
    }
    ...
    snprintf(outgoingPacket, 255, "%d,%d,%d", ballX, ballY, paddleX);
    udp.beginPacket(allowedClientIP, clientPort);
    udp.write((uint8_t*)outgoingPacket, strlen(outgoingPacket));
    udp.endPacket();
    ...
    if (packetSize) {
      ...
      String data = String(incomingPacket);
        ...
        if (firstComma != -1 && secondComma != -1) {
          ...
          opponentPaddleX = data.substring(secondComma + 1).toInt();
        } 
    }
    

    // Envoyer la position de la palette au serveur
    snprintf(outgoingPacket, 255, "%d", paddleX);
    udp.beginPacket(serverIP, serverPort);
        udp.write((uint8_t*)outgoingPacket, strlen(outgoingPacket));

    udp.endPacket();
    // Mise à jour de la position de la palette de l'adversaire sur le serveur
    if (isServer) {
      int packetSize = udp.parsePacket();
      if (packetSize) {
        udp.read(incomingPacket, 255);
        opponentPaddleX = String(incomingPacket).toInt();
      }
    }
    // Affichage
  display.clearDisplay();
